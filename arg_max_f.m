...
function output = arg_max_f(x1,Matriz,k)
x = N_k(x1,k);
tam = size(x,1);
...
posicao = -1;
% Pegar o indice que está a maior posição

tam2 = length(output_2);

%Pegar o indice na qual está a maior posicao do vetor da rota
for aux =1: tam
    if output_2(tam2) == output_1(aux)
        posicao = aux;
        break;
    end
end
